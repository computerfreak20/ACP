		...
		for i = 1,4 do
			...
			if unit == minion then
				...
				if apid and apid > 0 and (apid ~= _lastAttkpid or now()-_lastAttk > 5) then
					_lastAttk = now()
					_lastAttkpid = apid
					self:Chat('minionShot',_.s(self:_name(senderTweak),'damaged',(i==apid and 'own' or self:_name(i)..'\'s'),'crimp for',_.f(rDamage*10),'HP'))
				end
			end
		end
	end
	...
	for i = 1,4 do
		...
		if minion and minion ~= 0 then
			if alive(minion) and minion:character_damage()._health > 0 then
				...
				self:Chat('minionLost',_.s(self:_name(i),'lost a crimp to a',attacker,'.'))
			end
		end
	end
	-- ScanBuff
