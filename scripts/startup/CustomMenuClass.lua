if not CustomMenuClass then
	...
	function CustomMenuClass:openMenu(name, page)
		...
		if self.hasNavigation then
			...
			local buttonRectY = (self.panelNavigationDisplay:h() - 85)
			...
			self.panelNavigationDisplay:text({name = "close_menu_button_text", text = "Close Menu", valign = "center", align = "center", y = buttonRectY+2, layer = self.baseLayer + 4, color = self.menus[name].optionColor, font = self.menus[name].optionFont, font_size = self.menus[name].optionFontSize})
			...
			buttonRectX = (self.panelNavigationDisplay:w() / 2) - (buttonRectWidth / 2)
			buttonRectY = (self.panelNavigationDisplay:h() - 40)
			drawRect(self.panelNavigationDisplay, "keybind_menu_button_rect", buttonRectX, buttonRectY, buttonRectWidth, buttonRectHeight, self.menus[name].buttonRectHighlightColor, { rectLayer = self.baseLayer + 3 }):set_visible(false)
			self.panelNavigationDisplay:text({name = "keybind_menu_button_text", text = "Keybinding List", valign = "center", align = "center", y = buttonRectY, layer = self.baseLayer + 4, color = self.menus[name].optionColor, font = self.menus[name].optionFont, font_size = self.menus[name].optionFontSize})
			table.insert(self.activeButtons, { 
				text = "keybind_menu_button_text", 
				rect = "keybind_menu_button_rect",
				help = "Opens the Keybinding List",
				normalColor = self.menus[name].optionColor,
				highlightColor = self.menus[name].buttonHighlightColor,
				clbk = function() self:openMenu('keybind_menu', page) end
			})
